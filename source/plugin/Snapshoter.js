...
export default class Snapshoter {
    constructor(LSId) {
        ...
        this.save = function(data) {
            Snapshoter.check();
            if (data) {
                window.localStorage[this.storageId] = JSON.stringify(data);
            ...
            if (snapshoterList.length > 3) {
                ...
                window.localStorage.setItem('Snapshoter-List', snapshoterList);
    ...
    static check() {
        if (!window.localStorage || !this.storageId) {
            return;
        }
    }
    // 添加快照编号到队列
    ...
    static has(lsId) {
        Snapshoter.check();
        let snapshoterList = JSON.parse(window.localStorage.getItem('Snapshoter-List')) || [];
        return snapshoterList.includes(lsId.toString());
    };
};
