<template>
    <div class="customer-list"
        ...
        <div class="top-filter-box"
            ...
            <div flex="33"
                 class="sort bdr-right"
                 @click.stop="showSort = true"
                 layout="row"
        ...
        <!--排序-->
        <mt-popup class="popup-bottom ft-light"
                  v-model="showSort"
                  position="top">
            <p v-for="(item, index) in sortSlots"
               :key="index"
        ...
        <!--筛选-->
        <mt-popup class="popup-bottom filter-box"
                  v-model="showFilter"
                  position="top">
            <div class="category">
                ...
                <p class="ft-light">
                    <span v-for="(item, index) in category"
                          :key="index"
                          @click.stop="chooseCustomer(item)"
            ...
            <div class="zombie">
                ...
                <p class="ft-light">
                    <span v-for="(item, index) in customerFilterSlots.zombie.list"
                          :key="index"
                          @click.stop="filterClick(customerFilterSlots.zombie.field, item)"
                          :class="{active: param[customerFilterSlots.zombie.field] == item.value}">
                        {{item.label}}
                    </span>
                </p>
            </div>
            <div class="more">
                ...
                <p class="ft-light">
                    <span v-for="(item, index) in customerFilterSlots.hasCard.list"
                          :key="index"
                          @click.stop="filterClick(customerFilterSlots.hasCard.field, item)"
                          :class="{active: param[customerFilterSlots.hasCard.field] == item.value}">
                        {{item.label}}
                    </span>
                </p>
                <p class="ft-light">
                    <span v-for="(item, index) in customerFilterSlots.returnVisit.list"
                          :key="index"
                          @click.stop="filterClick(customerFilterSlots.returnVisit.field, item)"
                          :class="{active: param[customerFilterSlots.returnVisit.field] == item.value}">
                        {{item.label}}
                    </span>
                </p>
            </div>
            <div class="operate">
                ...
                <button class="color-primary"
                        @click="filterProfiles">确定</button>
            </div>
        </mt-popup>

        <!--搜索-->
        <mt-popup class="popup-bottom search-box"
                  v-model="showSearch"
                  position="top">
            <div layout="row"
            ...
            <div class="search-list">
                ...
                <p class="title"
                   v-for="(item, index) in searchProfilesList"
                   @click.stop="toDetail(item)"
                   :key="index">{{item.name}} - {{item.memberNo}}</p>
        ...
        <div class="profiles-box">
            <customer-profiles-cell @click.native="toDetail(item)"
                                    v-for="(item, index) in profilesList"
                                    :key="index"
                                    :value="item"
                                    :show.sync="item.show"></customer-profiles-cell>
            <div v-if="idLoading"
                 class="load-more color-tiffany-blue">
                <span style="display: inline-block;">
                    <mt-spinner color="#00CBC7"
                                type="fading-circle"></mt-spinner>
                </span>
            </div>
            <div v-else-if="profilesList.length < total"
                 class="load-more color-tiffany-blue">
                <span @click="loadMore">加载更多</span>
            </div>
            <div v-else
                 class="load-more color-gray">
                <span>下面没有啦╮(╯▽╰)╭</span>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        getCustomerListSearch() {
            var paramData = {
                query: [{ field: 'merchantId', value: this.$store.getters.merchantId }, { field: 'keyword', value: this.keyword, operation: 'like' }],
                page: 1,
                size: 20
            };
...
.customer-list {
    ...
    .top-filter-box {
        ...
        height: 50px;
        background: #ffffff;
        box-shadow: 0 1px 8px 0 rgba(142, 142, 142, 0.5);
        & > div {
            text-align: center;
            line-height: 28px;
        }
