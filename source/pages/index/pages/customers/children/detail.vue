        ...
        <div class="profiles background-cover">
            ...
            <div class="c-card"
                ...
                <div class="c-card-content"
                    ...
                    <img :src="dataModel.avatarId|mSrc(90,90,require('assets/imgs/avatar.png'))"
                         alt="">
                    <div>
...
export default {
    ...
    data() {
        return {
            ...
            childRoute: [
                { label: '账户资产', name: 'customer-assets' },
                { label: '护理记录', name: 'customer-record' },
                { label: '回访/关怀', name: 'customer-service' },
                { label: '档案信息', name: 'customer-intro' }
            ]
        };
    },
    watch: {
    ...
    methods: {
        fetchData() {
            this.$indicator.open();
            api_customerProfiles.memberDetail(this.$route.params.customerId).then(
                res => {
                    this.$indicator.close();
                    this.$store.commit('customerDetailUpdate', res.data);
                },
                err => {
                    this.$indicator.close();
                }
...
.customer-detail {
    ...
    .record-tab {
        height: 50px;
        background: #ffffff;
        box-shadow: 0 4px 4px 0 rgba(228, 228, 228, 0.5);
        & > div {
