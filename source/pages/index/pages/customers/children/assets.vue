        ...
        <div v-if="cards"
            ...
            <p class="title"
               ...
               @click="showMemberCard = !showMemberCard">
                <span>
                    <m-icon class="mr8"
                            link="icon-card" /> 会员卡</span>
                <span class="dark-gray ft12">
                    有效 <span class="color-primary ft16">{{cards.length || 0}}</span> 张
                    <span class="ml8 color-gray">
                        <m-icon :link="showMemberCard?'icon-arrow-up':'icon-arrow-down'" /></span>
                </span>
            </p>
            <transition name="slide-up"
                        mode="out-in">
                <div v-show="showMemberCard"
                     class="content">
                    <div v-for="(item, index) in cards"
                        ...
                        <p layout="row"
                           layout-align="space-between center">
                            <span class="no-wrap">{{item.name}} <span class="badge-card"
                                      :class="['back'+item.type]">{{cardType(item.type)}}</span></span>
                            <span v-if="cardSlots.includes(item.type)"
                                  class="ft12 dark-gray pull-right">剩余</span>
                        </p>
                        <p class="ft12 dark-gray"
                           layout="row"
                           layout-align="space-between center">
                            <span>有效期：{{item | validDate}}</span>
                            <span v-if="item.type==2"
                                  class="ft16 text-right no-wrap">{{item.balance/100 | currency}}</span>
                            <span v-else-if="item.type==4 || item.type==5"
                                  class="ft16 text-right no-wrap">{{item.balance || 0}}<span class="ft12 dark-gray">次</span></span>
        ...
        <div v-if="tickets"
            ...
            <p class="title"
               ...
               @click="showMemberCoupon = !showMemberCoupon">
                <span>
                    <m-icon class="mr8"
                            link="icon-coupon" /> 券</span>
                <span class="light-gray ft12">
                    有效 <span class="color-primary ft16">{{tickets.length}}</span> 张
                    <span class="ml8 color-gray">
                        <m-icon :link="showMemberCoupon?'icon-arrow-up':'icon-arrow-down'" /></span>
                </span>
            </p>
            <transition name="slide-up"
                        mode="out-in">
                <div v-show="showMemberCoupon"
                     class="content">
                        ...
                        <p class="ft12 dark-gray"
                           ...
                           layout-align="space-between center">
                            ...
                            <span v-if="item.type == 1"
                                  class="ft16 text-right no-wrap">{{item.balance/100 | currency}}</span>
                            <span v-else-if="item.type == 2 || item.type == 3"
                                  class="ft16 text-right no-wrap">{{item.balance || 0}}<span class="ft12 dark-gray">次</span></span>
                        </p>
                    </div>
                </div>
        ...
        <div v-if="presents"
            ...
            <p class="title"
               ...
               @click="showMemberPresents = !showMemberPresents">
                <span>
                    <m-icon class="mr8"
                            link="icon-gift" /> 赠品</span>
                <span class="light-gray ft12">
                    有效 <span class="color-primary ft16">{{presents.length}}</span> 份
                    <span class="ml8 color-gray">
                        <m-icon :link="showMemberPresents?'icon-arrow-up':'icon-arrow-down'" /></span>
                </span>
            </p>
            <transition name="slide-up"
                        mode="out-in">
                <div v-show="showMemberPresents"
                     class="content">
                    <div v-for="(item, index) in presents"
                         :key="index"
                         class="item">
                        <p layout="row"
                           layout-align="space-between center">
                        ...
                        <p class="ft12 dark-gray"
                           ...
                           layout-align="space-between center">
                            <span>有效期：{{item | validDate}}</span>
                            <span v-if="item.presentType == 1"
                                  class="ft16 text-right no-wrap">{{item.payRule.balance/100 | currency}}</span>
                            <span v-else
                                  class="ft16 text-right no-wrap">{{item.payRule.balance}}份</span>
                        </p>
                    </div>
                </div>
            </transition>
...
export default {
    ...
    methods: {
        fetchData() {
            this.isLoading = true;
            api_customerProfiles.memberDetailAssets(this.$route.params.customerId).then(
                res => {
    ...
    .card {
        ...
        .content {
            ...
            .item {
                ...
                box-shadow: 0 0 5px 0 rgba(217, 217, 217, 0.5);
                border-radius: 3px;
                padding: 8px 12px;
                margin: 12px;
                line-height: 24px;
